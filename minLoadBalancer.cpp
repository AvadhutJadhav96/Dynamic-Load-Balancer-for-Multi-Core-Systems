...
int main() {
    ...
    for (int i = 0; i < num_processes; ++i) {
        ...
        cin >> execution_time;
        cout << "Load (1 for Low, 2 for Medium, 3 for High): ";
        cin >> load;
        cout << "Priority (higher value = higher priority): ";
        cin >> priority;

        processes.emplace_back(id, arrival_time, execution_time, load, priority);
    }
    ...
    for (auto& proc : processes) {
        ...
        cout << "Process P" << proc.id << " arrived (Load: "
             << (proc.load == LOW_LOAD ? "Low" : proc.load == MEDIUM_LOAD ? "Medium" : "High")
             << ", Priority: " << proc.priority << ")\n";
        scheduler.assign_process_to_core(proc);
    }

    cout << "All processes assigned. Running scheduler...\n";
